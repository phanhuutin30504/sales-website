...
class CartController extends Controller
{
    ...
    public function create(Request $request)
    {
        ...
        if ($existingCartItem) {
            ...
            return back()->with('success', 'Sản phẩm đã được cập nhật trong giỏ hàng. Tổng giỏ hàng: ' . format_price($totalPrice));
        }

        $requestData = $this->prepareCartData($request, $userId, $productId, $quantity);
        $this->addNewCartItem($requestData);
        $totalPrice = $this->calculateTotalPrice($userId);
        return back()->with('success', 'Đã thêm vào giỏ hàng thành công. Tổng giỏ hàng: ' . format_price($totalPrice));
    }
