...
class ProductController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        $shoes = $products->filter(function($product) {
            return $product->unit->value === ProductUnit::Pail->value;
        });

        $accessories = $products->filter(function($product) {
            return $product->unit->value === ProductUnit::Bottle->value;
        });

        return view($this->getView()['index'], compact('breadcrums', 'products', 'shoes', 'accessories'));
    }
