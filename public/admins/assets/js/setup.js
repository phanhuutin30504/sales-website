...
function searchColumsDataTable(
    ...
    datatable
        ...
        .every(function () {
            ...
            if (
                column_date.length > 0 &&
                column_date.indexOf(column.selector.cols) !== -1
            ) {
                input.setAttribute("type", "date");
            } else if (
                (findColumnSelect = column_select.find(
                    (obj) => obj.column === column.selector.cols
                ))
            ) {
                input = document.createElement("select");
                createSelectColumnUniqueDatatableAll(
                    input,
                    findColumnSelect.data
                );
            } else if (
                (findColumnSelect2 = column_select2.find(
                    (obj) => obj.column === column.selector.cols
                ))
            ) {
                var resultColumnSelect2 = $.grep(
                    column_select2,
                    function (element) {
                ...
                if (resultColumnSelect2.length > 0) {
                    input = document.createElement("select");
                    if (
                        findColumnSelect2.ajax === true &&
                        findColumnSelect2.url
                    ) {
                        flagColSelect2Ajax = true;
                        input.setAttribute(
                            "class",
                            "form-select select2-bs5-ajax-many"
                        );
                        input.setAttribute("multiple", "true");
                        input.setAttribute("data-url", findColumnSelect2.url);
                    } else {
                        createSelect2ColumnDatatable(
                            input,
                            findColumnSelect2.data
                        );
                    }
                }
            }
...
$(document).on('click', '.open-modal-success', function () {
    var form = $("#modal-success"), action = $(this).data('route');
    form.attr('action', action);
});
